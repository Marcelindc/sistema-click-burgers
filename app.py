...
except: st.error("Erro no Banco de Dados. Verifique os Segredos."); st.stop()
...
with st.sidebar:
    ...
    if st.button("🚪 Sair (Logout)", use_container_width=True):
        ...
        puxar_dados.clear() 
        st.rerun()
    st.caption("Sistema ERP v4.1 - Click Burgers")
            ...
            with st.container(border=True):
                ...
                if st.button("✨ Finalizar Venda", use_container_width=True, type="primary"):
                    if subtotal_pedido == 0: st.error("Adicione lanches!")
                        ...
                        puxar_dados.clear() 
                        st.success(f"🎉 Venda salva com sucesso!")
                        time.sleep(1.5)
            ...
            for _, row in df_clientes.iterrows():
                try:
                    ...
                    if data_str and data_str != 'None':
                        ...
                        if dt_nasc.month == mes_atual: aniversariantes.append({'Nome': row['Nome'], 'Dia': dt_nasc.day, 'Whats': row.get('Telefone', '')})
                except: pass
            if aniversariantes:
    ...
    with st.form("form_despesas", clear_on_submit=True):
        ...
        if st.form_submit_button("💰 Lançar Despesa"):
            if descricao and valor_despesa > 0:
                ...
                puxar_dados.clear(); st.success(f"✅ Despesa lançada!")
    st.divider()
...
# -------------------------------------------------------------
    ...
    if not df_insumos.empty:
        ingredientes_escolhidos = st.multiselect("Ingredientes (Ficha Técnica):", df_insumos['Nome do Insumo'].tolist())
        ...
        if ingredientes_escolhidos:
            ...
            with col2: st.info(f"**Custo Ficha Técnica:**\n### R$ {custo_total_lanche:.2f}")
        ...
        with st.form("form_produtos", clear_on_submit=True):
            ...
            if st.form_submit_button("Salvar Produto") and nome_produto:
                ...
                puxar_dados.clear(); st.success("✅ Produto salvo!")
    st.divider() 
    df_produtos_edit = puxar_dados('Produtos')
    if not df_produtos_edit.empty:
        produto_selecionado = st.selectbox("Selecione Produto para Editar:", ["-- Escolha --"] + df_produtos_edit['Nome do Produto'].tolist())
        if produto_selecionado != "-- Escolha --":
            ...
            val_desc_atual = float(dados_atuais_p.get('Desconto', 0) if pd.notna(dados_atuais_p.get('Desconto')) else 0.0)
            ...
            with st.form("form_editar_produto"):
                ...
                if st.form_submit_button("Atualizar Produto"):
                    ...
                    puxar_dados.clear(); st.success("✅ Atualizado!")

# -------------------------------------------------------------
# MÓDULO 6: INSUMOS (FICHA TÉCNICA)
# -------------------------------------------------------------
elif menu_selecionado == "🍅 Insumos":
    st.title("🍅 Gestão de Insumos (Ficha Técnica)")
    st.caption("Cadastre aqui os insumos puros para calcular o custo dos seus lanches.")
    with st.form("form_insumos", clear_on_submit=True):
        nome_insumo = st.text_input("Nome do Insumo (Ex: Pão Brioche)")
        valor_compra = st.number_input("Valor da Compra / Pacote (R$)", min_value=0.0, step=0.01)
        rendimento = st.number_input("Rendimento (porções que esse pacote faz)", min_value=1, step=1)
        if st.form_submit_button("Salvar Insumo") and nome_insumo and valor_compra > 0 and rendimento > 0:
            ...
            puxar_dados.clear(); st.success(f"✅ Insumo salvo!")
    st.divider() 
    ...
    if not df_insumos_edit.empty:
        ...
        if insumo_selecionado != "-- Escolha --":
            ...
            with st.form("form_editar_insumo"):
                ...
                edit_valor = st.number_input("Novo Valor da Compra (R$)", value=float(dados_atuais['Valor de Compra']), step=0.01)
                edit_rend = st.number_input("Novo Rendimento (porções)", value=int(dados_atuais['Rendimento']), step=1)
                if st.form_submit_button("Atualizar e Recalcular Fichas"):
                        ...
                        for idx_prod, row_prod in df_produtos_recalc.iterrows():
                            try:
                                ...
                                if insumo_selecionado in receita or edit_nome in receita:
                                    ...
                                    val_desc_recalc = float(row_prod.get('Desconto', 0) if pd.notna(row_prod.get('Desconto')) else 0.0)
                                    novo_lucro = round((float(row_prod['Valor de Venda']) - val_desc_recalc) - novo_custo_total, 2)
                                    
                                    supabase.table("Produtos").update({"Custo Total": round(novo_custo_total, 2), "Lucro Bruto": novo_lucro}).eq("id", int(row_prod['id'])).execute()
                            except: pass 
                    puxar_dados.clear(); st.success("✅ Insumo atualizado e Lanches recalculados!")

        st.divider()
        st.markdown("#### Lista de Insumos")
        df_view = df_insumos_edit[['Nome do Insumo', 'Valor de Compra', 'Rendimento', 'Custo por Porção']].copy()
        st.dataframe(df_view, use_container_width=True, hide_index=True)

# -------------------------------------------------------------
# MÓDULO 7: LISTA DE COMPRAS (COM CORES, FILTROS E SEM DECIMAIS)
# -------------------------------------------------------------
    ...
    with col_l1:
        ...
        with st.form("form_lista_compras", clear_on_submit=True):
            ...
            compra_qtd = st.number_input("Quantidade em Estoque", min_value=0, step=1, value=0)
        ...
        if not df_compras_edit.empty:
            ...
            if item_selecionado != "-- Escolha --":
                ...
                with st.form("form_edit_compras"):
                    ...
                    edit_qtd_comp = st.number_input("Quantidade em Estoque", value=int(dados_item.get('Quantidade', 0)), step=1)
    ...
    with col_l2:
        ...
        if not df_compras_view.empty:
            # Tratamento para evitar erros de leitura e forçar os tipos de dados
            df_compras_view['Quantidade'] = pd.to_numeric(df_compras_view['Quantidade'], errors='coerce').fillna(0).astype(int)
            df_compras_view['Valor'] = pd.to_numeric(df_compras_view['Valor'], errors='coerce').fillna(0.0)
            ...
            filtro_prioridade = st.radio(
                "Filtrar por prioridade de compra:", 
                ["🛒 Mostrar Todos", "🔴 Apenas Críticos (< 2)", "🟡 Atenção (2 a 5)"], 
                horizontal=True
            )
            
            # Filtro inteligente
            if filtro_prioridade == "🔴 Apenas Críticos (< 2)":
                df_compras_view = df_compras_view[df_compras_view['Quantidade'] < 2]
            elif filtro_prioridade == "🟡 Atenção (2 a 5)":
                df_compras_view = df_compras_view[(df_compras_view['Quantidade'] >= 2) & (df_compras_view['Quantidade'] <= 5)]
            ...
            if not df_compras_view.empty:
                def colorir_estoque(valor):
                    try:
                        ...
                        elif v >= 2: return 'background-color: #fff3cd; color: #856404; font-weight: bold;'
                        else: return 'background-color: #f8d7da; color: #721c24; font-weight: bold;'
                    except: return ''

                formato_tabela = {'Valor': 'R$ {:.2f}', 'Quantidade': '{:.0f}'}
                df_estilizado = df_compras_view.drop(columns=['id']).style.map(colorir_estoque, subset=['Quantidade']).format(formato_tabela)
                st.dataframe(df_estilizado, use_container_width=True, hide_index=True)
            else:
                st.warning("Nenhum produto encontrado neste filtro.")
